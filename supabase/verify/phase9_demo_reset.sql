...
-- Repeated reset must run in a separate transaction: reset_demo_data uses
-- pg_try_advisory_xact_lock, which is held until the outer transaction ends.
begin;
...
begin
  ...
  insert into public.operator_profiles (id, display_name, is_active)
  values (v_operator_id, 'Phase 9 Operator', true)
  on conflict (id) do update set is_active = excluded.is_active;
